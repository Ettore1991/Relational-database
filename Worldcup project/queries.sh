...
echo "$($PSQL "SELECT MAX(winner_goals) FROM games GROUP BY winner_id ORDER BY MAX(winner_goals) DESC LIMIT 1;")"

echo -e "\nNumber of games where the winning team scored more than two goals:"
echo  "$($PSQL "SELECT COUNT(*) FROM games WHERE winner_goals > 2;")"

echo -e "\nWinner of the 2018 tournament team name:"
echo  "$($PSQL "SELECT name FROM teams t LEFT JOIN games g ON t.team_id = g.winner_id WHERE year = 2018 AND round = 'Final';")"
